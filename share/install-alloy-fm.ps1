...
if ($ATTR -eq $null -or $ATTR -eq "") {
	Write-Host "ERROR: Required argument ATTR missing. Must contain attributes key pair."
	exit 1
}
$NEED_PROXY = "true"
$PROXY_URL = ""
...
try {
	...
	if ($NEED_PROXY -eq "false") {
		...
		Invoke-WebRequest -Uri $CONFIG_URI -Outfile $CONFIG_FILE -ErrorAction Stop -Proxy $PROXY
	}	
